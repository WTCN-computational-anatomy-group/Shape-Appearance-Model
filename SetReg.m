function [EA,B,lb_qA,lb_pA] = SetReg(EZZ,N,s)
% Set the regularisation of Z
% FORMAT [EA,B,lb_qA,lb_pA] = SetReg(EZZ,N,s)
%
...
B       = eye(K);
EA      = inv((EZZ+max(diag(EZZ))*1e-5*eye(size(EZZ)))/N);
...
if false
    ...
    lb_qA    = 0.5*(nu -K-1)*ElndetA - 0.5*trace(Lambda \EA)...
              -0.5*nu *K*log(2) - 0.5*nu *LogDet(Lambda ) - MultiGammaLn(nu /2,K);
    lb_pA    = 0.5*(nu0-K-1)*ElndetA - 0.5*trace(Lambda0\EA)...
              -0.5*nu0*K*log(2) - 0.5*nu0*LogDet(Lambda0) - MultiGammaLn(nu0/2,K);

   % Double check!
   lb_pA  = lb_pA + 0.5*N*LogDet(EA); % Adjusted this because  lb_L includes LogDet(EA)
else
    lb_qA = 0;
    lb_pA = 0;
end
