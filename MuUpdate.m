function [mu,lb_pmu] = MuUpdate(mu,gmu,Hmu,N,s)
% Update the mean
% FORMAT [mu,lb_pmu] = MuUpdate(mu,gmu,Hmu,N,s)
%
% mu     - The mean
% gmu    - 1st derivatives
% Hmu    - 2nd derivatives
...
case {'normal','gaussian','laplace','binomial','binary'}
    ...
    for l=1:d(4)
        ...
        dmu         = spm_field(Hmu(:,:,:,l), gr,                      [s.vx N*s.mu_settings s.mg_its]);
        mu(:,:,:,l) = mu(:,:,:,l) - dmu;
...
case {'multinomial','categorical'}
    ...
    dmu    = spm_field(Hmu, gr,             [s.vx N*s.mu_settings s.mg_its]);
    mu     = mu - dmu;
